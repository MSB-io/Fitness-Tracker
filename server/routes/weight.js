...
router.get("/latest", protect, async (req, res) => {
  try {
    const latest = await WeightLog.findOne({ user: req.user._id }).sort({
      date: -1,
    });
...
router.post("/", protect, async (req, res) => {
  try {
    const { weight, bodyFat, muscleMass, waterPercentage, notes, date } =
      req.body;
...
router.put("/:id", protect, async (req, res) => {
  try {
    ...
    log = await WeightLog.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
      runValidators: true,
    });
