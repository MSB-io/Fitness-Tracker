...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // Get recent data
      const [recentWorkouts, recentMeals, weightHistory, goals] =
        await Promise.all([
  ...
  async (req, res) => {
    try {
      ...
      if (client.role !== "user") {
        return res
          .status(400)
          .json({ message: "Can only assign regular users as clients" });
      }
...
router.post("/plans", protect, authorize("trainer"), async (req, res) => {
  try {
    ...
    if (!client) {
      return res
        .status(404)
        .json({ message: "Client not found or not assigned to you" });
    }
