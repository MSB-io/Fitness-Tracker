...
router.get("/me", protect, async (req, res) => {
  try {
    const user = await User.findById(req.user._id).populate("assignedTrainer", "name email")
    res.json(user)
  } catch (error) {
    res.status(500).json({ message: "Server error", error: error.message })
  }
...
router.get("/trainers", protect, async (req, res) => {
  try {
    const trainers = await User.find({ role: "trainer" }).select("name email profile createdAt")
    res.json(trainers)
  } catch (error) {
    res.status(500).json({ message: "Server error", error: error.message })
  }
...
router.post("/trainer-request", protect, async (req, res) => {
  try {
    ...
    if (req.user.assignedTrainer) {
      return res.status(400).json({ message: "You already have an assigned trainer" })
    }
    ...
    if (existingRequest) {
      return res.status(400).json({ message: "You already have a pending request to this trainer" })
    }
