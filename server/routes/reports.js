...
router.get("/summary", protect, async (req, res) => {
  try {
    ...
    const workoutStats = {
      ...
      avgWorkoutsPerWeek: (workouts.length / ((end - start) / (7 * 24 * 60 * 60 * 1000))).toFixed(1),
    }

    // Nutrition stats
    const nutritionStats = {
      avgDailyCalories:
        meals.length > 0 ? Math.round(meals.reduce((sum, m) => sum + m.totalCalories, 0) / meals.length) : 0,
      avgDailyProtein:
        meals.length > 0 ? Math.round(meals.reduce((sum, m) => sum + m.totalProtein, 0) / meals.length) : 0,
    ...
    const weightProgress =
      weightLogs.length >= 2
        ? {
            ...
            change: weightLogs[weightLogs.length - 1].weight - weightLogs[0].weight,
            trend: weightLogs,
          }
        : null
