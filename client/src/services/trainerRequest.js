...
export const trainerRequestService = {
  ...
  requestTrainer: async (trainerId, message) => {
    const response = await api.post("/auth/trainer-request", { trainerId, message })
    return response.data
  },
  ...
  approveRequest: async (requestId, responseMessage) => {
    const response = await api.put(`/trainer/requests/${requestId}/approve`, { responseMessage })
    return response.data
  },

  // Trainer-side: Reject request
  rejectRequest: async (requestId, responseMessage) => {
    const response = await api.put(`/trainer/requests/${requestId}/reject`, { responseMessage })
    return response.data
  },
}
