...
const WorkoutDetail = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-primary">{workout.exercises?.length || 0}</p>
          </div>
        </Card>
        <Card className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-primary">{workout.totalDuration} min</p>
          </div>
        </Card>
        <Card className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-primary">{workout.totalCaloriesBurned} kcal</p>
          </div>
        </Card>
      </div>
              ...
              <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <span
                      className={`inline-block px-2 py-0.5 text-xs rounded-full ${getCategoryColor(exercise.category)}`}
                    >
                      {exercise.category}
                    </span>
                  </div>
                ...
                <div className="flex items-center gap-6 text-sm">
                  {exercise.sets > 0 && (
                    <div className="text-center">
                      <p className="font-medium text-primary">{exercise.sets}</p>
                      <p className="text-muted">sets</p>
                    </div>
                  )}
                  {exercise.reps > 0 && (
                    <div className="text-center">
                      <p className="font-medium text-primary">{exercise.reps}</p>
                      <p className="text-muted">reps</p>
                    </div>
                  )}
                  {exercise.weight > 0 && (
                    <div className="text-center">
                      <p className="font-medium text-primary">{exercise.weight}</p>
                      <p className="text-muted">kg</p>
                    </div>
                  )}
                  {exercise.caloriesBurned > 0 && (
                    <div className="text-center">
                      <p className="font-medium text-primary">{exercise.caloriesBurned}</p>
      ...
      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} title="Delete Workout" size="sm">
        <p className="text-muted mb-6">Are you sure you want to delete this workout? This action cannot be undone.</p>
